...
available_datasets = [
    ...
    'drunk-dataset' #Drunk, Sober Dataset
]
...
def load_df(dfdc_df_path: str, ffpp_df_path: str, dfdc_faces_dir: str, ffpp_faces_dir: str, dataset: str) -> (pd.DataFrame, str):
    if dataset.startswith('dfdc'):
        ...
        root = ffpp_faces_dir
    elif dataset.startswith('drunk-'):
        df = pd.read_pickle(ffpp_df_path) # 기존과 구별하기 위해 새로운 변수명(경로) 생성 필요
        root = dfdc_faces_dir
    else:
...
def get_split_df(df: pd.DataFrame, dataset: str, split: str) -> pd.DataFrame:
    if dataset == 'dfdc-35-5-10':
        if split == 'train':
            split_df = df[df['folder'].isin(range(170))]
            # split_df = df[df['folder'].isin(range(140))]
        elif split == 'val':
            split_df = df[df['folder'].isin(range(175, 200))]
            # split_df = df[df['folder'].isin(range(140, 160))]
        elif split == 'test':
            split_df = df[df['folder'].isin(range(200, 250))]
            # split_df = df[df['folder'].isin(range(160, 200))]
        else:
...
def make_splits(dfdc_df: str, ffpp_df: str, dfdc_dir: str, ffpp_dir: str, dbs: Dict[str, List[str]]) -> Dict[str, Dict[str, Tuple[pd.DataFrame, str]]]:
    ...
    """
    # split_name: train/val, split_dbs: train_dataset(dfdc-35-5-10, drunk-)/val_dataset(dfdc-35-5-10, drunk)
    split_dict = {}
    full_dfs = {}
    for split_name, split_dbs in dbs.items():
        ...
        for split_db in split_dbs:
            ...
            split_dict[split_name][split_db] = (split_df, root) #train/val, DataFrame 튜플 => split_dict[train][dfdc] = (split_data, data_dir)형식으로 저장되게끔
